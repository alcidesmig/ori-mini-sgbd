...
node_position btree_find_node(node_t* node, int key) {
	...
	int pos;
	if (node_find_key(node, key, &pos)) {
		// Se a chave foi encontrada nesse nó,
		// retorne um meio de acessá-la
		return node_position_new(node, pos);
	}
	else {
		...
		if (node->is_leaf) {
			...
			return btree_find_node(node->children[pos], key);
		}
	}
}
...
node_position btree_insert(BTree* bt, int key, void *value) {
	...
	pair_t *pair = pair_new(key, value);
	...
	if (root->n_keys == 2*bt->order -1) {
		...
		node_t *new_root = node_new(bt->order, FALSE);
		new_root->children[0] = root;

		btree_split(new_root, 0, bt->order);
		bt->root = new_root;

		// Podemos prosseguir com a inserção
		return btree_insert_nonfull(new_root, pair, bt->order);
	}
	else {
		...
		return btree_insert_nonfull(root, pair, bt->order);
	}
}

void btree_split(node_t *node, int pos, int order) {
	...
	node_t *z = node_new(order, y->is_leaf);
	z->n_keys = order-1;

	#if DEBUG
	printf("moving keys from root to right: ");
	#endif
	node_deslocate_keys_up(z, y, 0, order-1, 0, order);

	if (!y->is_leaf) {
		#if DEBUG
		printf("moving children from root to right: ");
		#endif
		node_deslocate_children_up(z, y, 0, order, 0, order);
	}
	y->n_keys = order-1;
	...
	#endif
	node_deslocate_children_down(node, node, node->n_keys, pos, 1, 0);
	...
	#endif
	node_deslocate_keys_down(node, node, node->n_keys-1, pos-1, 1, 0);
	...
	if (node->is_leaf) {
		...
		if (pos >= 0 && pair->key == node->keys[pos]->key) {
			...
			if (node->children[pos]->n_keys == 2*order-1) {
				btree_split(node, pos, order);
				if (pair->key > node->keys[pos]->key) {
					pos++;
				}
			}

			return btree_insert_nonfull(node->children[pos], pair, order);
		}
	}
}
...
node_position btree_remove(BTree* bt, int key) {
	...
	node_position pos = btree_remove_node(bt->root, key, bt->order);
...
node_position btree_remove_node(node_t *node, int key, int order) {
	...
	if (node_find_key(node, key, &pos)) {
		...
		if (node->is_leaf) {
			...
			#endif
			node_deslocate_keys_up(node, node, pos, node->n_keys, 0, 1);
			...
			if (left->n_keys >= order) {
				...
				node_position max = node_find_max(left);
				pair_t *p = pair_copy(max.node->keys[max.index]);

				node->keys[pos] = p;

				btree_remove_node(left, p->key, order);
				return node_position_new(node, pos);
			}
				...
				node_position min = node_find_min(right);
				pair_t *p = pair_copy(min.node->keys[min.index]);
				node->keys[pos] = p;

				btree_remove_node(right, p->key, order);
				return node_position_new(node, pos);
			}
			else {
				/*
				...
				node_deslocate_keys_up(left, right, 0, order-1, order, 0);
				node_deslocate_children_up(left, right, 0, order, order, 0);
				left->n_keys = 2*order - 1;

				node_delete(right);

				node_deslocate_keys_up(node, node, pos, node->n_keys-1, 0, 1);
				node_deslocate_children_up(node, node, pos, node->n_keys-1, 1, 2);
				node->n_keys--;

				return btree_remove_node(left, key, order);
			}
		}
	}
	else {
		...
		if (next->n_keys == order-1) {
			...
			if (left->n_keys >= order) {
				...
				#endif
				btree_insert_nonfull(right, node->keys[pos-1], order);
				...
				return btree_remove_node(right, key, order);
			}
			else if (right->n_keys >= order) {
				/*
				...
				#endif
				btree_insert_nonfull(left, node->keys[pos-1], order);
				...
				node_deslocate_keys_up(node, node, pos, node->n_keys-1, 0, 1);
				node_deslocate_children_up(node, node, pos+1, node->n_keys, 0, 1);
				node->n_keys--;

				node_deslocate_keys_up(left, right, 0, order-1, order, 0);
				node_deslocate_children_up(left, right, 0, right->n_keys+1, order-1, 0);
				node_delete(right);
		...
		return btree_remove_node(next, key, order);
	}
}
